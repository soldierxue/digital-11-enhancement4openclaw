...
输出目录结构:
output/
├── [provider]_[earliest]_to_[latest]/    # RAW 原始下载（按邮箱和日期范围命名）
│   ├── 原始附件1.pdf
│   ├── 原始附件2.zip → 解压取 PDF
...
│   └── other/
│       └── 20260205_发票_unknown.pdf
├── summary.json                          # 结构化汇总
└── summary.md                            # 人类可读汇总报告

RAW 文件夹命名示例:
- [163]_[2026.02.01]_to_[2026.03.31]/
- [gmail]_[2026.01.15]_to_[2026.03.10]/
```
...
信息提取优先级:
1. Kiro CLI 发票识别（OCR/AI）— 对 PDF 文件调用 `kiro-cli chat` 提取日期、金额、供应商、地点
2. 邮件元数据 — 发件人、主题、日期
3. 文件名模式 — 原始文件名中的关键词
4. 兜底 — 下载日期 + 默认类型
